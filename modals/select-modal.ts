...
export class DGSelectModal extends Modal {
	...
	selectedOptions: Set<AgentOption> = new Set();

	onSubmit?: (selectedOptions: AgentOption[]) => Promise<void>;
	...
	onOpen() {
		...
		new ButtonComponent(contentEl).setButtonText("Close").onClick(() => {
			this.close();
		});
	}
}
